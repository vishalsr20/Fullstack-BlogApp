...
const GetAllBlogs = () => {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      const blogs = data.getBlogs || data.blogs || data || [];
      setAllBlogs(blogs);
  ...
  return (
    <div className="bg-gray-200 flex flex-col mt-20 items-center w-full min-h-screen py-8 px-4">
      <div className="w-[90%] max-w-6xl">
        <h2 className="text-5xl font-bold text-center bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent mb-8">
          ✨ All Blogs ✨
        </h2>
        ...
        {allBlogs.length === 0 ? (
          ...
          <div className="flex flex-col space-y-8">
            {allBlogs.map((blog) => (
              <div
                key={blog._id}
                className="flex flex-col lg:flex-row w-full bg-white p-6 rounded-2xl shadow-2xl transition-all duration-500 cursor-pointer hover:-translate-y-1 hover:scale-[1.01]"
                onClick={() => Category(blog._id)}
              >
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                ...
                {blog.image && (
                  <div className="w-full lg:w-1/3 flex justify-end mt-4 lg:mt-0">
                    <img
                      src={blog.image}
                      alt={blog.title}
                      className="rounded-2xl object-cover w-full lg:w-[300px] h-[200px]"
                    />
                  </div>
                )}
              </div>
